...
let conteudo = item.innerHTML;

for (i in ITENS_LOJA) {
    let dados = ["nome", "preco", "descricao"];

    if (i != 0) {
        item = document.createElement("div");
        document.getElementById("secao-venda").appendChild(item);
        item.innerHTML = conteudo;
    }

    document.getElementsByClassName("imagem")[i].setAttribute("src", ITENS_LOJA[i].url_img);

    for (j in dados) {
        document.getElementsByClassName(dados[j])[i].innerHTML = ITENS_LOJA[i][dados[j]];
    }
}